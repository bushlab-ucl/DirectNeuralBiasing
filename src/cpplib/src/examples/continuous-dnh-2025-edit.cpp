...
int CHANNEL_NUM = 1;  // Channel number to record from
int SLEEP_TIME = 250; // Necessary to prevent race conditions. Vary param based on system CPU power.

int main(int argc, char *argv[])
{
  cout << "Starting Blackrock continuous data collection from channel " << CHANNEL_NUM << "..." << endl;
  ...
  // Setup the specified channel (continuous recording at 30kHz)
  ...
  res = cbSdkGetChannelConfig(0, CHANNEL_NUM, &chan_info);
  if (res != CBSDKRESULT_SUCCESS)
  {
  ...
  {
    cout << "ERROR: Channel " << CHANNEL_NUM << " does not exist" << endl;
    cbSdkClose(0);
    return 1;
  }
  ...
  {
    cout << "ERROR: Channel " << CHANNEL_NUM << " does not support analog input" << endl;
    cbSdkClose(0);
    return 1;
  }
  ...
  res = cbSdkSetChannelConfig(0, CHANNEL_NUM, &chan_info);
  if (res != CBSDKRESULT_SUCCESS)
  {
  ...
  while (loop_count < 10)
  {
    ...
    if (res == CBSDKRESULT_SUCCESS)
    {
      // Check if we have data for our channel
      // Need to find which index corresponds to our channel
      for (uint32_t i = 0; i < trial.count; i++)
      {
        if (trial.chan[i] == CHANNEL_NUM && trial.num_samples[i] > 0)
        {
          cout << "Loop " << loop_count << ": Received "
               << trial.num_samples[i] << " samples from channel "
               << trial.chan[i] << endl;

          INT16 *samples = (INT16 *)trial.samples[i];

          // Print first few samples as verification
          int samples_to_print = min((uint32_t)10, trial.num_samples[i]);
          ...
          total_samples_received += trial.num_samples[i];
          break; // Found our channel, no need to continue
        }
      }
