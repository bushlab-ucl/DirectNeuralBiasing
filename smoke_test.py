#!/usr/bin/env python3
"""DNB smoke test — quick sanity check for offline and live pipelines.

Generates a synthetic 1 Hz slow wave signal, runs the detection pipeline,
and confirms that STIM events are produced. Optionally connects to live
hardware (NPlay or Cerebus) for a timed test run.

Usage:
    python smoke_test.py                           # offline only (no hardware)
    python smoke_test.py --live nplay              # + live NPlay
    python smoke_test.py --live nplay --channel 5  # single channel
    python smoke_test.py --live cerebus --inst-addr 192.168.0.1
    python smoke_test.py --no-live                 # skip live explicitly
    python smoke_test.py --seconds 30              # longer live run

Place pink_noise_short.wav in assets/ for audio stimulation.
"""
...
logger = logging.getLogger("dnb.smoke_test")
...
def run_offline_test():
    ...
    if stim1:
        print("  PASS — slow wave stimulation working")
    else:
        print("  WARN — no stimulation events detected")
...
if __name__ == "__main__":
    ...
    logger.info("DNB v%s — smoke test", dnb.__version__)
